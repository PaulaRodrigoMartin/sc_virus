mireact <- function(exp, motifs=7, 
	species="hs", seq.type="utr3", out.file = "res.data", out.meonly = T,	verbose = F , install.dir = "~/temp/miRNAsc", mail = NULL, memory="64g", wt="72:00:00", tarbaserun=FALSE, acc="sc_covid_PiB2023", annotation = NULL){
	#################################
...
sink("sbatch.script")
cat("#!/bin/sh","#SBATCH --nodes 1","#SBATCH -c 1",paste0("#SBATCH --time ",wt),paste0("#SBATCH --account ",acc),paste0("#SBATCH --mem ",memory),"#SBATCH --job-name Rscript",sep="\n")
