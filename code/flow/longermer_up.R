...
subs_dfs_paths_up <- gsub("annot4_", "longermer_upreg_", celltypes)
...
cell_up <- cell[cell$diffexpressed_corr == "UP_corrected",]

cell_down_log2FC <- cell_up[order(cell_up$log2fc, decreasing = TRUE), ]
g <- rownames(cell_down_log2FC[1:100,])
cell_down_corrviralload <- cell_up[order(cell_up$corr_viralload, decreasing = TRUE), ]
h <- rownames(cell_down_corrviralload[1:100,])
cell_up$logttest <- -log10(cell_up$ttest_pval_corrected)
cell_down_ttest <- cell_up[order(cell_up$logttest, decreasing = TRUE), ]
y <- rownames(cell_down_ttest[1:100,])

unique_rownames <- unique(c(g,h,y))

cell_down_top <- cell_up[unique_rownames,]

print("upregulated longermers doing...")
## UP_REGULATED kmers
dict_up <- list()
...
dict_up <- longmers
#summary(dict)
print("upregulated longermers done!")

saveRDS(dict_up, file = subs_dfs_paths_up)
