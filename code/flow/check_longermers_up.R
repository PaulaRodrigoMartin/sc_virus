...
subs_dfs_paths_up <- gsub("longermer_upreg_", "longermer_upreg_filtered_", downreg)

# Get 7mers from where the longermers have been constructed
cell_down <- cell[cell$diffexpressed_corr == "UP_corrected",]
...
for (i in longmers){
  ...
  quantile_value <- quantile(corr_values_random$Freq, 0.5) #threshold
