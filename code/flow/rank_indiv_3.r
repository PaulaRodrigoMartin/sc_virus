...
a <- b
a <- a[order(a$corr_viralload, decreasing = FALSE), ]
...
cand_plasm <- a_100[a_100$log2fc < summary(a_100$log2fc)[2],]
...
## has hits in the sarscov2 genome
cand_plasm_vir <- cand_plasm[cand_plasm$sarshitnumber_rev == 0 | cand_plasm$sarshitnumber > 0,]
print(paste("final dim after checking sars hit: ", dim(cand_plasm_vir))) 

d <- cand_plasm_vir[,c(1, 8, 14, 23, 25, 26, 31, 33)]
...
if (nrow(d)>0){
    saveRDS(cand_plasm_vir, file = subs_dfs_paths_1)
} else {
